...
* @retval      无
*/
uint8_t BlueInit(void){
	uint8_t ret = 0;
	
	atk_mw579_init(ATK_MW579_UART_BAUDRATE_115200);
	atk_mw579_enter_config_mode();
	ret  = atk_mw579_set_name(BLE_NAME);								//设置ATK-MW579蓝牙名称
	ret += atk_mw579_set_hello(BLE_HELLO);							//设置ATK-MW579开机欢迎语
	...
	ret += atk_mw579_set_adptim(5);						//设置ATK-MW579广播速度
